  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::dsr_msgs::EnableAlterMotionRequest_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.limit_dPOS.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.limit_dPOS[i]);
    }
    if (v.limit_dPOS.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "limit_dPOS_per: ";
    if (v.limit_dPOS_per.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.limit_dPOS_per.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.limit_dPOS_per[i]);
    }
    if (v.limit_dPOS_per.empty() || true)
      s << "]";
  }
