...
struct Printer< ::dsr_msgs::WriteDataRTRequest_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::dsr_msgs::WriteDataRTRequest_<ContainerAllocator>& v)
  {
    if (false || !indent.empty())
      s << std::endl;
    s << indent << "external_force_torque: ";
    if (v.external_force_torque.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.external_force_torque.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.external_force_torque[i]);
    }
    if (v.external_force_torque.empty() || true)
      s << "]";
    ...
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "external_analog_input: ";
    if (v.external_analog_input.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.external_analog_input.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.external_analog_input[i]);
    }
    if (v.external_analog_input.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "external_analog_output: ";
    if (v.external_analog_output.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.external_analog_output.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<double>::stream(s, true ? std::string() : indent + "    ", v.external_analog_output[i]);
    }
    if (v.external_analog_output.empty() || true)
      s << "]";
  }
