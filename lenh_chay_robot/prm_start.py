...
def add_box(scene, name, size, position, rpy, frame_id):
    ...
    rospy.loginfo(f"✅ Added '{name}' at {position}")

def main():
    rospy.init_node("move_sequence_prm_star", anonymous=True)
    ...
    group.set_planner_id("PRMstarkConfigDefault")  # ✅ Đây là PRM*
    ...
    rospy.loginfo("🚀 Executing motion using PRM*")
    for idx, target in enumerate(sequence):
        group.set_joint_value_target(target)
        plan = group.plan()
        if plan and plan[0]:
            rospy.loginfo(f"➡️ Moving to point {idx+1} with PRM*")
