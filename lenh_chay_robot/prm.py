...
def main():
    rospy.init_node("move_sequence_with_prm", anonymous=True)
    moveit_commander.roscpp_initialize(sys.argv)

    # Khởi RobotCommander, PlanningScene và MoveGroup với namespace và robot_description của robot thật
    ...
    # 19 hộp theo tọa độ bạn cung cấp
    boxes = [
    ...
    # Chờ tối đa 5s để các hộp xuất hiện
    ...
    # Cấu hình MoveGroup dùng PRM
    group.set_goal_joint_tolerance(0.01)
    group.set_max_velocity_scaling_factor(0.2)
    group.set_max_acceleration_scaling_factor(0.2)
    group.set_planning_time(10.0)                 # tăng thời gian cho PRM tìm đường
    group.set_planner_id("PRMkConfigDefault")     # chuyển sang thuật toán PRM
    ...
    rospy.loginfo("🚀 Executing motion with PRM planner")
    for idx, target in enumerate(sequence):
        group.set_joint_value_target(target)
        plan = group.plan()
        if plan and plan[0]:
            rospy.loginfo(f"➡️ Moving to point {idx+1} using PRM")
            group.execute(plan[1], wait=True)
            group.stop()
            group.clear_pose_targets()
            rospy.sleep(1.0)
        else:
            rospy.logwarn(f"⚠️ PRM planning failed at point {idx+1}")
            break
